...
def main():
    ...
    if cfg["scan_all"]:
        ...
        if not pairs:
            ...
            # falls back to the static, pre-curated Binance.US-reachable
            # list (config/settings.yaml's `pairs:`). A full-universe
            # rotation was tried here (2026-07-18) but reverted: GitHub
            # throttles scheduled runs on this repo to an effective
            # ~55-90 minute cadence regardless of configured interval
            # (confirmed via 4 canary workflows), so rotation's fast-cycle
            # design never actually delivered, and most of the extra ~371
            # pairs beyond this list aren't reachable from GitHub Actions
            # without a working proxy anyway (~73% no_data when tried).
